...
## `DisplaySettings.AllowUserResizing` Default — Flip to `false` When Fixed Window Size Is Set?
**Priority: Discussion — not starting yet.** `DisplaySettings.AllowUserResizing` defaults to `true`, which fights the fixed-canvas pattern: a game that sets `PreferredWindowWidth/Height` to lock the window to a specific size still gets the resize-tracking behavior unless it *also* sets `AllowUserResizing = false`. Surfaced 2026-04-25 by `samples/ShmupSpace` BlazorGL conversion — after locking the canvas DOM and drawing buffer to 720×960, browser resizes still shifted/squashed content because `HandleClientSizeChanged` kept firing (`AllowUserResizing` was still `true`).

Two design options:

- **Auto-default `AllowUserResizing` to `false` when `PreferredWindowWidth.HasValue`.** Makes the obvious intent ("I want a 720×960 window") imply the obvious default ("the player can't resize it"). Backward-compat risk: existing samples that set `PreferredWindowWidth` *and* expect the user to be able to resize would silently change behavior. None known today.
- **Just flip the default outright to `false`.** The current default is the wrong choice for most desktop games (most don't want users dragging window borders to break their layout) and clearly the wrong choice for KNI BlazorGL fixed-canvas. Stretch-to-viewport games would have to opt in to `true`. Bigger blast radius but cleaner long-term default.

Decide when next BlazorGL or fixed-window desktop sample lands. Either change requires a `tests/FlatRedBall2.Tests/` audit for assumptions about the default.
