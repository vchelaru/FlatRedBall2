...
## Climb-Down from Standing at Ladder Top
**Priority: Soon — natural follow-up to step-off-top.** When a player is standing on a platform directly above a ladder, pressing Down should enter climbing mode and let them descend. Currently the climb-down-from-top trigger (`IsLadderBelowFeet`) only fires when the player is already on the ground collision that sits flush with the ladder top — it does not fire from a general "standing" state. Coordinate with the existing grounded + Down enter trigger in `PlatformerBehavior`.
