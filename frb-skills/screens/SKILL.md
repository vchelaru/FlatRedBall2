...
- **No persistent cross-screen UI (sidebar nav, shared HUD shell) — by design.** `MoveToScreen` tears down all Gum content along with the old screen; there is no chrome/host layer that survives the swap. For UI that should appear on every screen, re-add the same Gum component in each screen's `CustomInitialize` (a shared base class works well) rather than building a persistent-chrome or push/pop-style host — that was proposed and rejected to keep `MoveToScreen`'s full-teardown-and-rebuild model the only lifecycle to reason about.
