...
- **`.tmx`** — `TileMap.TryReloadFrom` patches tile IDs and rebuilds every TSC registered via `GenerateCollisionFromClass`/`Property`, including shapes sourced from object-layer rectangles/polygons. Returns `false` on structural change (map/layer/tileset diff). Hand-authored mutations on a generated TSC are **wiped** — put augmentations in `CustomInitialize`.
  - **Object-layer content is compared by count only, not by position/size/rotation.** If you move or resize an object-layer rectangle/polygon without adding or removing one, `TryReloadFrom` reports `true` (compatible) but the live map's object layers are never re-synced from disk — only tile-layer cell data is patched in place. The result: the TSC (and any entities spawned via `CreateEntities` from that same object layer) keep the **stale pre-edit shape/position** after a "successful" reload. If your object layers can be edited independently of the tile layers, don't rely on in-place reload to pick that up — force `RestartScreen(RestartMode.HotReload)` for object-layer edits, or add/remove a throwaway object alongside the real edit to force the count check to fail and trigger a restart.
