...
## Tile Collision (TileShapes)

`TileShapes` implements `ICollidable` and works as static geometry with the `entities, staticGeometry` overload:
...
// TileShapes has a dedicated overload — no explicit type arguments needed
AddCollisionRelationship(_playerFactory, tiles)
    .MoveFirstOnCollision();
```

`SolidSides` on adjacent tiles are maintained automatically — interior shared edges are cleared so entities glide across flat surfaces without snagging on seams.

### SlopeMode (per-relationship)

When one side is a `TileShapes` containing polygon (slope) tiles, `relationship.SlopeMode` controls how overlap is resolved:
...
**Prefer `TileShapes` over individual wall entities for static level geometry.** Individual entities sharing edges will cause the player to snag on seams between adjacent tiles because each entity maintains its own `SolidSides` independently. `TileShapes` solves this by automatically suppressing interior shared edges. Use individual wall entities only when tiles need independent behavior (e.g., destructible blocks, moving platforms) — and for grid-shaped arrangements of those, set `Factory<T>.IsSolidGrid = true` so the factory applies the same seam suppression across adjacent cells (see `entities-and-factories` skill).
...
```csharp
var cloudTiles = new TileShapes { GridSize = 16f };
...
playerVsClouds.CanDropThrough = true; // opt in to Down+Jump drop-through for clouds
...
For sloped one-way tiles (polygon cells in the `TileShapes`) you must also set `SlopeMode = PlatformerFloor`. Without it the collection falls back to SAT, the slope-aware LastPosition gate doesn't run, and the player falls through while walking uphill.
...
var playerVsClouds = AddCollisionRelationship(_playerFactory, cloudTiles);
playerVsClouds.OneWayDirection = OneWayDirection.Up;
playerVsClouds.CanDropThrough = true;
...
### `CanDropThrough` — cloud platforms vs. hard one-way barriers

`OneWayDirection` and player drop-through are intentionally separate concerns:

- `CanDropThrough = true` — this relationship honors
...
- `CanDropThrough = false` (default) — player drop-through input is ignored; the relationship always blocks in the configured direction. **Use for hard one-way barriers** (e.g. Yoshi's Island ratchet doors) that should never be passable the wrong way.
...
```csharp
Body = new AARect { Width = 14, Height = 14 };
...
_footProbe = new AARect { Width = 2, Height = 2, Y = -1 };
