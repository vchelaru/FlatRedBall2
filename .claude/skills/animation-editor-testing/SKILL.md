...
## The update check is one mechanism now, not two — test it as `IApplicationUpdater`

Both the persistent startup banner and the About dialog's "Check for Updates"/"Restart Now" (issue #1033) drive the exact same `IApplicationUpdater.DownloadUpdateAsync` (`AnimationEditor.App/Services/VelopackApplicationUpdater.cs`) through `MainWindow.RunApplicationUpdateCheckAsync`. There is no separate GitHub-release-comparison path to force a fake "update available" state via a `-p:Version` override anymore (that whole subsystem — `UpdateChecker`/`UpdateCheckResult`/`UpdateCheckPolicy` — was deleted as a duplicate implementation). Cover `NoUpdate`/`ReadyToRestart`/`Failed` states headlessly with `FakeApplicationUpdater` (`AnimationEditor.App.Tests/TestHelpers.cs`) — see `UpdateCheckStartupTests.cs` and `AboutDialogTests.cs` for the pattern. To exercise the real Velopack path end-to-end against a local release feed instead of GitHub, point `ANIMATION_EDITOR_TEST_UPDATE_SOURCE` (Debug builds only, see `ApplicationUpdateSource.ForCurrentBuild`) at one; building that feed is a `vpk`/Velopack releasify step outside this skill's scope.

## A test `.achx` opened through the real load path needs `CoordinateType = Pixel`
