...
- **Source code** — class outlines, property lists, method signatures, call sites. XML docs on a member count as source — point at it, don't restate it.
- **`docs/` GitBook tree** (has its own `SUMMARY.md`) — user-facing behavior, engine APIs, tutorials.
...
1. Read the relevant source files and their XML docs.
2. Check `docs/SUMMARY.md` for existing coverage of the topic — including whether the finding is a specific instance of a general principle another skill already documents (see "Converging pull" above).
3. Skim a few existing skills under `.claude/skills/` (and `frb-skills/` if present) to match style and depth.
...
- Folder name must match `name` (kebab-case noun phrases, e.g. `game-lifecycle`, `shaders`). No `anthropic`/`claude` in the name.
...
- **Drop boilerplate.** No "Reference guide for…", no "Load this when working on…", no "Covers FRB2's…". The fact that this is a skill is implicit.
...
✅ "FRB2 shader compilation via the render loop. Triggers: .fx tabs, Content.Load<Effect>, ShaderCompiler."
...
- Use real file paths and symbols from this repo. Skills age badly when they describe imaginary code — FRB2 is not public and the engine is expected to churn, so a misleading name is API feedback (file the fix), not something to paper over with a workaround skill.
...
- Pointers: links to `docs/` pages, key source files, and related skills.
...
- Anything already in source or its XML docs — link instead of restating full class outlines.
- Code examples unless the snippet captures an irreplaceable pattern spanning 2+ classes in a non-obvious sequence.
