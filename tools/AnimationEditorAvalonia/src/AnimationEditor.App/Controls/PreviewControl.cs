...
public class PreviewControl : Control
{
    ...
    // ── Shape drag ────────────────────────────────────────────────────────────
    private object? _draggingShape;   // AxisAlignedRectangleSave | CircleSave
    private float   _shapeDragStartX; // shape X before drag began
    private float   _shapeDragStartY; // shape Y before drag began
    private Point   _shapeDragAnchor; // screen point where drag began
    ...
    /// <summary>
    /// Returns the shape (AxisAlignedRectangleSave or CircleSave) at screen position
    /// (<paramref name="px"/>, <paramref name="py"/>), or <c>null</c> when nothing is hit.
    /// The currently-selected shape is tested first. Only shapes on the selected frame are eligible.
    /// Ruler-strip clicks (px &lt; RulerSize or py &lt; RulerSize) always return null.
    /// </summary>
    private object? HitTestShape(float px, float py)
    {
        ...
        object? TestRect(AxisAlignedRectangleSave r)
        {
            float sx = cx + r.X * om;
            float sy = cy - r.Y * om;
            float hw = MathF.Max(r.ScaleX * om, 4f);
            float hh = MathF.Max(r.ScaleY * om, 4f);
            const float tol = 4f;
            return px >= sx - hw - tol && px <= sx + hw + tol &&
                   py >= sy - hh - tol && py <= sy + hh + tol ? r : null;
        }

        object? TestCircle(CircleSave c)
        {
            float sx = cx + c.X * om;
            float sy = cy - c.Y * om;
            float hitRadius = MathF.Max(c.Radius * om, 8f);
            float dx = px - sx, dy = py - sy;
            return dx * dx + dy * dy <= hitRadius * hitRadius ? c : null;
        }

        // Selected shape has priority
        if (SelectedState.Self.SelectedRectangle is { } selR && TestRect(selR) is not null) return selR;
        if (SelectedState.Self.SelectedCircle    is { } selC && TestCircle(selC) is not null) return selC;

        foreach (var r in frame.ShapeCollectionSave.AxisAlignedRectangleSaves)
            if (TestRect(r) is { } hit) return hit;
        foreach (var c in frame.ShapeCollectionSave.CircleSaves)
            if (TestCircle(c) is { } hit) return hit;
    ...
    private void CommitShapeDrag()
    {
        ...
        if (MathF.Abs(newX - _shapeDragStartX) > eps || MathF.Abs(newY - _shapeDragStartY) > eps)
        {
            ...
            if (frame is not null)
                UndoManager.Self.Record(new MoveShapeCommand(
        ...
        // Re-assign selection to trigger property-panel refresh
        if (_draggingShape is AxisAlignedRectangleSave rs) SelectedState.Self.SelectedRectangle = rs;
        else if (_draggingShape is CircleSave cs)          SelectedState.Self.SelectedCircle    = cs;
    ...
    /// <summary>
    /// Test-only: applies a world-space delta to the currently-selected shape and commits
    /// it as if the user dragged and released. Records an undo entry when the position changes.
    /// </summary>
    internal void SimulateShapeDrag(float worldDx, float worldDy)
    {
        var shape = SelectedState.Self.SelectedShape;
        if (shape is null) return;

        _draggingShape = shape;
        if (shape is AxisAlignedRectangleSave r)
        {
            _shapeDragStartX = r.X; _shapeDragStartY = r.Y;
            r.X += worldDx; r.Y += worldDy;
        }
        else if (shape is CircleSave c)
        {
            _shapeDragStartX = c.X; _shapeDragStartY = c.Y;
            c.X += worldDx; c.Y += worldDy;
        }
        CommitShapeDrag();
    }
    ...
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        // Selected shape takes priority over guide drag
        ...
        if (hitShape is not null)
        {
            ...
            if (hitShape is AxisAlignedRectangleSave hr) SelectedState.Self.SelectedRectangle = hr;
            else if (hitShape is CircleSave hc)          SelectedState.Self.SelectedCircle    = hc;

            _draggingShape   = hitShape;
            _shapeDragAnchor = pos;
            if (hitShape is AxisAlignedRectangleSave sr2) { _shapeDragStartX = sr2.X; _shapeDragStartY = sr2.Y; }
            else                                          { var sc = (CircleSave)hitShape; _shapeDragStartX = sc.X; _shapeDragStartY = sc.Y; }
            e.Pointer.Capture(this);
            return;
        }
    ...
    protected override void OnPointerMoved(PointerEventArgs e)
    {
        ...
        if (_draggingShape is not null)
        {
            ...
            if (om > 1e-6f)
            {
                ...
                float dy = -(float)(pos.Y - _shapeDragAnchor.Y) / om; // Y-up: screen-Y increase = world-Y decrease
