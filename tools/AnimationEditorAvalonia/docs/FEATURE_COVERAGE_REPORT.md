...
| PL08b | User-placed guides visibility (#689) | Toggles rendering/hit-testing of user-placed horizontal/vertical guide lines; controlled by `ShowUserGuidesCheck`, only shown once a guide exists; placing a new guide while hidden reveals them all again (matches Photoshop). The toggle itself is runtime-only and resets each session — only the guide-line *positions* are persisted in `AESettingsSave`, not this visibility flag (confirmed while wiring browser settings persistence, #754: zero test coverage or references to a persisted visibility flag in `MainWindow.ApplyCompanionSettings`) |
