...
`.achj` file's animation chains into the currently open tileset, as Tiled tile
animations (`Tile.frames`).

## This is a lossy conversion

Tiled's tile-animation model is strictly poorer than `.achj`: a tile's `frames` is just
`{ tileId, duration }[]`, where every frame must be another tile *already in the same
tileset, on the same grid*. There's no per-frame flip, offset, tint, or cross-texture
frame, and no concept of a named animation chain - Tiled animates from a starting tile,
not a name.

What this importer does about that, per chain:

- Matches each `.achj` frame's pixel rect to a tile in the open tileset by position.
  Frames must be `"coordinateType": "Pixel"` (not `"UV"`), grid-aligned, and exactly
  one tile in size - anything else is skipped with a warning.
- Skips frames whose `textureName` doesn't match the tileset's image (a `.achj` chain
  can span multiple textures; Tiled tile animations can't).
...
Copy this folder into Tiled's extensions directory, then **Edit > Reload Extensions**:
...
With a tileset open in Tiled: **Edit > Import .achj Animation Frames...**, pick a
`.achj` file. Warnings and a summary go to the Console view (**View > Views and
Toolbars > Console**) and a dialog.

## Development

`achj-mapper.mjs` is the pure conversion logic (no dependency on Tiled's `tiled`/
`Tileset`/`TextFile` scripting globals) and is unit tested under plain Node:
